...
primary_sales.specfic.store1 <-
  primary_sales.specfic[which(primary_sales.specfic$Store_Code=='Store3'),]
primary_sales.specfic.store1 <-
...
#Weekly data is difficult to work 
#Number of seasonal period is both large and non-integer
#Average 52.18 week per year - Approx : 52
...
#Plot for the sales quantity 
#Analysis for store level
primary_sales.specfic.store1%>%
  ggplot(aes(y=x,x=""))+
  # theme_minimal()+
...
modelseries=primary_sales.specfic.store1$x%>%tk_ts()
...
p+geom_point(aes(y=x,x=Date,color="Above or below IQR"),size=4,data = Data3,shape=1)+
  ...
  labs(x="Time",fill="",title="Sales Record for Store 1")+
